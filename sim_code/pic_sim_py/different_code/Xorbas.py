...
class Xorbas(Code_Placement):
    ...
    def generate_best_placement(self):
        if self.d - 1 >= self.r:
            new_cluster = cluster(self.d - 1, len(self.best_placement['raw_information']))
            ...
            for group in self.stripe_information[0:-1]:
                ...
                if last_cluster.remaind() < len(group) - 1:
                    last_cluster = cluster(self.d - 1, len(self.best_placement['raw_information']))
        ...
        if global_ptr < remain:
            ...
            for item in global_parity[global_ptr:]:
                each_cluster.add_new_block(item, self.block_to_groupnumber[item])
                self.best_placement['block_map_clusternumber'][item] = each_cluster.return_id()
